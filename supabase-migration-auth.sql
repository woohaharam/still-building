...
--
-- 이전 상태: anon key만 있으면 누구나 글·일정을 쓰고 지울 수 있었어요.
-- 이후 상태: 아래에 적은 '주인 이메일'로 로그인한 사람만 쓰기가 가능해요.
--
-- ⚠️ 실행 전에 딱 하나 바꿔주세요 — 바로 아래 is_owner() 안의 이메일.
--    Supabase 대시보드 Authentication > Users 에서 만든 계정의 이메일이어야 해요.
--    안 바꾸고 실행하면 아무도 글을 못 쓰게 되니(로그인은 되는데 저장이 실패),
--    그때는 이 파일의 is_owner()만 고쳐서 다시 실행하면 됩니다.
-- ════════════════════════════════════════════════════════════
...
--     — 켜져 있으면 아무나 계정을 만들 수는 있어요. 만들어도 is_owner()가
--       막아주지만, 애초에 못 만들게 하는 편이 깔끔해요.
-- (c) Storage > Policies 에서 예전에 만들어둔 '전부 허용' 정책이 남아 있으면 삭제
--     — 정책은 OR로 합쳐지기 때문에, 느슨한 게 하나라도 남으면 그게 이깁니다.
