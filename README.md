...
- **해결** 색만 조정. CSS 변수 한 곳이라 세 줄 수정으로 끝
- **성과** **접근성 96 → 100점.** 같은 김에 폰트 `@import` → `<link rel="preconnect">` 로 옮겨 요청 왕복 1회 감소
- **남은 문제** 이때는 `--paper` 기준으로만 맞췄다. 카드 배경인 `--surface` 위에서는 여전히 미달이었다는 걸 나중에 알았다 → 8번

</details>

<details>
<summary><b>8. 눈으로 고친 대비가 카드 위에서는 여전히 미달이었다</b> — 배경을 하나만 보고 맞춤</summary>

- **문제** 6번에서 대비를 고쳤는데도 `/service` 에서 흐린 글자 7곳이 기준 미달로 잡힘
- **원인** 색을 `--paper` 하나만 기준으로 맞췄다. 카드 배경인 `--surface` 는 글자와 더 가까워서 같은 색이 **4.61 → 4.23:1** 로 떨어진다. 화면으로는 구분이 안 되는 차이다
- **시도** 없음. 눈으로는 못 찾는 문제라, 애초에 찾은 방법이 해결의 일부다 — 헤드리스 브라우저에 axe-core 를 넣어 **11개 페이지 × 라이트·다크** 를 한 번에 훑었다
- **해결** 기준을 `--paper` 가 아니라 더 빡빡한 `--surface` 로 바꿔 색을 다시 계산. 겸사겸사 달력의 지난달·다음달 날짜(`opacity-30`, 대비 **1.9:1**)도 나왔다. 흐리게 보이는 것과 못 읽는 것은 다르다 — 글자는 `--ink-muted` 로 올리고 배경 색만 옅게 남겼다
- **성과** 11개 페이지 × 두 테마에서 **위반 0**. 6번은 Lighthouse 가 검사한 두 페이지만 통과한 것이었다
...
app/                    페이지 (App Router)
  ...
  ├ books/              독후감 (목록 + 상세)
  ├ travel/             여행 기록 (목록 + 상세)
  ├ service/            복무 기록 — 헤더에 없고 바닥글에서만 간다
  ├ calendar/           일정 + 글 쓴 날
...
6. Settings > API 에서 `Project URL`과 `anon public key` 복사
