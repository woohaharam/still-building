/*
  화면에 쓰는 문체
  ────────────────
  한 문단 안에서 문체가 섞이면 글이 덜 다듬어져 보인다. 앞 문장과 뒷 문장의
  말끝이 서로 다른 자리가 페이지마다 있었다.

  자리에 따라 하나를 고르고, 그 안에서는 끝까지 지킨다.

    합니다체 — 나를 소개하는 자리. 랜딩 · 소개 · 프로젝트 · 이력서 · 개인정보.
               처음 보는 사람이 읽는 글이라 차분한 쪽이 맞다.

    해요체   — 내 기록을 다루는 자리와 안내 문구. 캘린더 · 복무 · 근무 ·
               관리자 화면, 그리고 "아직 없어요" 같은 빈 화면 문구.

  둘을 가르는 기준은 읽는 사람이다. 앞쪽은 남이 보는 글이고 뒤쪽은 내가
  쓰는 화면이다.
*/
