...
export const PROJECTS: Project[] = [
  {
    ...
    features: [
      ...
      },
      {
        ...
        body: '핀을 누르면 그 여행기로 간다. 처음에는 나라 윤곽선을 미리 SVG 로 구워 그렸다. 밖으로 나가는 요청이 없고 자바스크립트 없이도 보인다는 게 좋았는데, 정작 좌표를 고르는 관리자 화면에서 지명도 길도 없으니 어디를 누르는지 알 수가 없었다. 그래서 실제 타일 지도로 갈았다. 타일은 밝고 어두운 판이 다 있는 것을 골라 사이트 테마를 따라가게 했고, 테마가 바뀌는 건 <html> 의 class 를 지켜봐서 안다. 처음 고른 판은 그래프를 얹을 배경으로 만든 것이라 일부러 색이 빠져 있었는데, 지도 자체가 주인공인 화면에는 맞지 않아 길과 지명이 또렷한 쪽으로 바꿨다. 타일을 못 받아오면 회색 판만 남아 방문자가 고장인지 원래 그런지 알 수 없으므로, 여러 장이 연달아 실패하면 그렇게 말한다. 지도가 못 떠도 그 아래 목록에 여행이 전부 있어서 읽는 데는 지장이 없다. 바꾸면서 쓰지 않게 된 투영 코드와 48KB 짜리 윤곽선 데이터를 지웠다. 나라별 중심점만 남겼는데, 좌표를 적지 않은 여행이 거기 찍힌다.',
      },
      {
