...
export const PROJECTS: Project[] = [
  {
    ...
    features: [
      ...
      {
        title: '외부 요청 없이 그리는 여행 지도',
        body: '다녀온 곳에 핀을 찍고 누르면 그 여행기로 간다. 타일 서버를 붙이면 방문자 IP 가 남의 서버로 가고 CSP 에 구멍을 하나 더 뚫어야 해서, 나라 윤곽선을 빌드 전에 SVG 경로로 구워두고 그것만 그린다. 실행 중 나가는 요청이 없고 서버에서 그려지니 자바스크립트 없이도 보이며, 색이 CSS 변수를 따라가 다크 모드에 저절로 맞는다. 좌표를 고르는 관리자 화면은 반대로 실제 타일 지도를 쓴다. 윤곽선만 있는 지도 위에서는 어디를 누르는지 알 수가 없어서 좌표를 고르는 도구로는 쓸모가 없었다. 로그인해야 들어오는 자리라 나가는 요청이 내 것뿐이고, 지명이 보이는 값이 훨씬 크다. 공개 화면과 관리자에 서로 다른 지도를 두는 편이 한쪽으로 통일하는 것보다 낫다고 봤다. 구우면서 날짜변경선을 넘는 나라가 지도를 가로지르는 줄로 그려지는 문제와, 같은 이유로 러시아 중심점이 태평양(202도)에 잡히는 문제를 잡았다.',
      },
      {
    ...
    troubles: [
      ...
      {
        title: '지도를 가로지르는 줄 하나와, 태평양에 찍힌 러시아',
        problem:
          '여행 지도를 구웠더니 세계 지도를 왼쪽 끝에서 오른쪽 끝까지 가로지르는 가느다란 줄이 하나 그어졌다. 같은 자료로 뽑은 나라별 중심점에서는 러시아가 경도 202도, 피지가 경도 11도로 나왔다. 202도는 애초에 존재하지 않는 좌표고, 11도는 아프리카다.',
        cause:
          '둘 다 날짜변경선 때문이었다. 180도를 넘나드는 나라는 좌표가 180에서 -180으로 튄다. 그 두 점을 직선으로 이으면 지도를 가로지르는 줄이 되고, 그대로 평균을 내면 중심점이 지구 반대편으로 간다.',
        tried:
          '튀는 구간에서 선을 끊어봤다. 줄은 사라졌는데 그 나라만 반쪽이 뜯긴 채 남았다.',
        solution:
          '앞 점과의 차이가 180도를 넘으면 한 바퀴를 더하거나 빼서 좌표를 이어붙인다. 중심점은 그 상태로 계산하고 결과만 -180~180 안으로 되돌린다. 윤곽선은 화면에 걸치는 만큼 한 바퀴씩 옮겨 여러 번 그려서, 진짜 세계 지도처럼 양쪽 가장자리에 나뉘어 나오게 했다.',
        result:
          '러시아 99.9도, 피지 178도로 제자리를 찾았다. 중심점 174개가 전부 지구 안에 있고 지도 위에 그려지는지 검사하는 테스트를 붙였다. 중심점 문제는 화면을 보기 전에 이 테스트가 먼저 잡았다.',
      },
      {
