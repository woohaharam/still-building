...
export const PROJECTS: Project[] = [
  {
    ...
    features: [
      ...
      },
      {
        ...
        body: '다녀온 곳에 핀을 찍고 누르면 그 여행기로 간다. 타일 서버를 붙이면 방문자 IP 가 남의 서버로 가고 CSP 에 구멍을 하나 더 뚫어야 해서, 나라 윤곽선을 빌드 전에 SVG 경로로 구워두고 그것만 그린다. 실행 중 나가는 요청이 없고 서버에서 그려지니 자바스크립트 없이도 보이며, 색이 CSS 변수를 따라가 다크 모드에 저절로 맞는다. 핀 자리는 등장방형 도법으로 계산하는데, 관리자에서 지도를 눌러 좌표를 고르는 쪽은 그 역함수를 쓴다. 구우면서 날짜변경선을 넘는 나라가 지도를 가로지르는 줄로 그려지는 문제와, 같은 이유로 러시아 중심점이 태평양(202도)에 잡히는 문제를 잡았다.',
      },
      {
