...
/**
 * 복무 정보. 날짜가 바뀌면 여기만 고치면 화면 전체가 따라간다.
 *
 * 부대를 비행단까지만 적는다. 이력을 보는 사람에게 필요한 건 어느 군, 어느
 * 병과, 무슨 일을 했느냐지 편제의 어디에 속했느냐가 아니다. 그 아래 단위까지
 * 적으면 이력서에 보태는 것 없이, 근무 명세서(/service/duty)와 겹쳐 읽었을 때
 * 누가 언제 어디에 서 있었는지만 좁혀진다.
 *
 * 더 적고 싶으면 unit 한 줄을 늘리면 된다.
 */
export const SERVICE = {
  branch: '공군',
  /** 충주에 있는 비행단. 부대 이름 자체는 공개된 것이다. */
  unit: '제19전투비행단',
  /** 병과. 이력을 훑는 사람이 눈으로 찾는 낱말이다. */
  specialty: '군사경찰',
  /** 임명받은 직책. */
  role: '기동순찰병',
  /**
